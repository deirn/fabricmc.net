...
export async function generateMixin(writer: TemplateWriter, options: ComputedConfiguration) : Promise<unknown[]> {
    ...
    await writer.write(`src/main/java/${packageName.replaceAll("\.", "/")}/${className}.java`, render(mixinTemplate, {
        className,
        packageName
    }));
...
export async function generateClientMixin(writer: TemplateWriter, options: ComputedConfiguration) : Promise<unknown[]> {
    ...
    await writer.write(`src/client/java/${packageName.replaceAll("\.", "/")}/${className}.java`, render(mixinClientTemplate, {
        className,
        packageName
    }));
