...
async function generateJavaEntrypoint(writer: TemplateWriter, options: ClassOptions): Promise<unknown> {
    ...
    await writer.write(`src/main/java/${options.path}.java`, render(javaEntrypointTemplate, options))

    if (options.clientEntrypoint) {
        await writer.write(`src/client/java/${options.path}Client.java`, render(javaEntrypointClientTemplate, {...options, className: options.className + "Client"}));
    ...
    if (options.dataEntrypoint) {
        await writer.write(`src/main/java/${options.path}DataGenerator.java`, render(javaEntrypointDataGeneratorTemplate, {...options, className: options.className + "DataGenerator"}));
...
async function generateKotlinEntrypoint(writer: TemplateWriter, options: ClassOptions): Promise<unknown> {
    ...
    await writer.write(`src/main/kotlin/${options.path}.kt`, render(kotlinEntrypointTemplate, options))

    if (options.clientEntrypoint) {
        await writer.write(`src/client/kotlin/${options.path}Client.kt`, render(kotlinEntrypointClientTemplate, {...options, className: options.className + "Client"}))
    ...
    if (options.dataEntrypoint) {
        await writer.write(`src/main/kotlin/${options.path}DataGenerator.kt`, render(kotlinEntrypointDataGeneratorTemplate, {...options, className: options.className + "DataGenerator"}))
